        ...
        public class SceneController : MonoBehaviour
        {
            ...
            private PageController menu
            {
                get 
                {
                    ...
                    if (m_Menu == null)
                    {
                        LogWarning("You are trying to access the page controller, but no instance was found!");
            ...
            private IEnumerator LoadScene()
            {
                ...
                string _taregtSceneName = SceneTypeToString(m_TargetScene);
                SceneManager.LoadScene(_taregtSceneName);
            }
            ...
            private bool SceneCanBeLoaded(SceneType _scene, bool _reload)
            {
                ...
                {
                    LogWarning("You are trying to load a scene [" + _scene + "] that is already loaded");
                    return false;
                }
                else if (_targetSceneName == string.Empty)
                {
                    LogWarning("The scene you are trying to load [" + _scene + "] is invalid");
                    return false;
                }
                else if (m_SceneIsLoading)
                {
                    LogWarning("Unable to load scene [" + _scene + "]. Another scene is loading.");
                    return false;
                }
            ...
            {
                if (!debug) return;
                Debug.LogError("[Scene Controller]: " +_msg);
            }
            #endregion
